...
$menutype = 'content';
//判断是否是审核yuan 
$shenfen = $dsql->GetOne("SELECT `mtype`,`rank` FROM #@__member WHERE mid='$cfg_ml->M_ID'");
/*-------------
function _ShowForm(){  }
--------------*/
if(empty($dopost))
{
    ...
    include(DEDEMEMBER."/mystyles/jianzhi_edit.htm");
    exit();
}
...
else if($dopost=='save')
{
    ...
    $upQuery = "UPDATE `#@__archives` SET
              ...
              mtype = '$mtypesid',        
              flag='$flag'
     WHERE id='$aid' and  channel=$channelid";
    }
	else
	{
		 //更新数据库的SQL语句
   ...
   //判断是否是审核员
	if($shenfen['rank'] == '20000')
	{
		ShowMsg("修改信息成功","shenhe.php?channelid={$channelid}","0",1000);
	}else
	{
