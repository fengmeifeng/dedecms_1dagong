...
'$color','$writer','$source','$litpic','$pubdate','$senddate','$mid','$description','$keywords','$mtypesid'); ";
    ...
    if(empty($addtable))
    {
		...
		$edu=$_POST['edu'];
		$sex=$_POST['sex'];
		$gzjingyan=$_POST['gzjingyan'];
		$pingjia=$_POST['pingjia'];
		$zwleixing=$_POST['zwleixing'];
		...
		*/
        ...
        //$inquery = "INSERT INTO `{$addtable}`(aid,typeid,userip,redirecturl,templet{$inadd_f}) Values('$arcID','$typeid','$userip','',''{$inadd_v})";
       	$inquery = "INSERT INTO `{$addtable}`(aid,typeid,userip,redirecturl,templet{$inadd_f},zhiwei,pay,edu,sex,gzjingyan,pingjia,zwleixing,age) Values('$arcID','$typeid','$userip','',''{$inadd_v},'$zhiwei','$pay','$edu','$sex','$gzjingyan','$pingjia','$zwleixing','$age')";
	    if(!$dsql->ExecuteNoneQuery($inquery))
        {
            $gerr = $dsql->GetError();
            $dsql->ExecuteNoneQuery("Delete From `#@__archives` where id='$arcID'");
   ...
   /* $msg = "
	...
	ShowMsg("发布简历信息成功","jianli.php?channelid={$channelid}","0",1000);
}