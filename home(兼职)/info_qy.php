...
if($dopost=='save'){
	if(!empty($_POST)){
		...
		$query = "UPDATE `#@__member` set uname='".$linkman."',sphone='".$phone."',sex='".$sex."', shengfenid='".$shengfenid."', spacesta='2',verify='1' WHERE mid='{$cfg_ml->M_ID}'";
		//echo $query;exit;
		if($dsql->ExecuteNoneQuery($query)){
			...
			$querys = "UPDATE `{$membermodel->table}` set sex='".$sex."',linkman='".$linkman."',email='".$email."',zhiwei='".$zhiwei."',company='".$company."',mobile='".$phone."',address='".$address."' WHERE mid='{$cfg_ml->M_ID}'";
			...
			if(!$dsql->ExecuteNoneQuery($querys)){
				...
				/*------------------------------*/
				ShowMsg('成功更新你的详细资料！','index_company.php',0,5000);
				exit();
			}
		}
	}
