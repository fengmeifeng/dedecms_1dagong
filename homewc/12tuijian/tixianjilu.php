<?php
	...
	if(!empty($user)){
		...
		$num="SELECT * FROM tixian WHERE userbianhao='".$user['bianhao']."' ";
		$num=$db->get_total($num);
		$page=new page($num,10,$_SERVER['SCRIPT_NAME']);
		$sql="SELECT * FROM tixian WHERE userbianhao='".$user['bianhao']."' ORDER BY `add_time` DESC  LIMIT ".$page->limit;
		$tujiandata=$db->getall($sql);
		require_once(dirname(__FILE__)."/style/tixianjilu.htm");
