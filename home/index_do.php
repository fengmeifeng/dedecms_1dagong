...
if($fmdo=='sendMail')
{
    ...
    ShowMsg('成功发送邮件，请稍后登录你的邮箱进行接收！', '/home');
    exit();
}
...
else if($fmdo=='user')
{
    ...
    else if($dopost=="regnew")
    {
        ...
        //require_once(dirname(__FILE__)."/baoming.php");
		require_once(dirname(__FILE__)."/p_register.php");
        exit();
    }
	
	 //引入企业注册页面
    else if($dopost=="regnew1")
    {
...
else if($fmdo=='login')
{
    ...
    if($dopost=="login")
    {
		...
		*/
		//echo CheckUserID($userid,'',false);exit;
        ...
        if(defined('UC_API') && @include_once DEDEROOT.'/uc_client/client.php')
        {
            ...
            if($uid > 0) {
                ...
                if(!$rs) {
					...
					//修改rank
					//判断个人和企业
					 $panduan1 = $dsql->GetOne("SELECT `mtype` FROM #@__member WHERE mid='$cfg_ml->M_ID'");
					 
                   		if($panduan1['mtype'] == "企业"){
						  $mt="`mtype`='企业'";
						 }
						 else{
						 $mt="`mtype`='个人',`rank`='10',`sex`='男'";
						 }
				   
				    $res = $dsql->ExecuteNoneQuery("INSERT INTO #@__member SET $mt,`userid`='$username',`pwd`='$password',`uname`='$username',`money`='$money', `email`='$email', `scores`='$scores', `matt`='0', `face`='',`safequestion`='0',`safeanswer`='', `jointime`='$jointime',`joinip`='$joinip',`logintime`='$logintime',`loginip`='$loginip';");
        ...
        {	
            ...
            if(empty($gourl) || preg_match("#action|_do#i", $gourl))
            {
				...
				$num=10;
				setcookie("num", $num, time()+3600*24);		//设置cookie值发送短信次数，徐阳添加-----------
				if($cfg_ml->IsLogin()){
					
					//判断个人和企业
					 $panduan2 = $dsql->GetOne("SELECT `mtype` FROM #@__member WHERE mid='$cfg_ml->M_ID'");
					 //echo $panduan2['mtype'];exit;
					 if($panduan2['mtype'] == "企业"){
						 ShowMsg("成功登录，5秒钟后转向系统主页...","./index_company.php",0,2000);
						 }
						 else{
...
else
{
	...
	//echo $row['mtype'];exit;
	if($panduan2['mtype'] == "企业"){
    ShowMsg("本页面禁止返回!","index_company.php");
	}else
	{
