...
$type = empty($type)? "0" : trim($type);
$tpl = '';
$menutype = 'mydede';
$rank = empty($rank)? "" : $rank;
//echo $type;echo $cfg_ml->M_ID;exit;
if($rank == 'top'){
    ...
    $sql = "SELECT s.*,t.* FROM `#@__member_stow` AS s left join `#@__member_stowtype` AS t on t.stowname=s.type  where type=".$type." order by s.addtime desc";
    $tpl = 'zwgenzong';
}
...
$dlist->pageSize = 20;
$dlist->SetTemplate(DEDEMEMBER."/templets/zwgenzong.htm");
$dlist->SetSource($sql);
$dlist->Display();