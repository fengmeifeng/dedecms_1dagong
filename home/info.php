...
if($dopost=='save'){
	if(!empty($_POST)){
		...
		*/
		...
		$query = "UPDATE `#@__member` set uname='".$uname."',sphone='".$phone."',sex='".$sex."', shengfenid='".$shengfenid."', spacesta='2',verify='1' WHERE mid='{$cfg_ml->M_ID}'";
		...
		//==-----------------------------
			//echo $nativeplace;exit;
			//$querys = "UPDATE `{$membermodel->table}` set sex='".$sex."',syear='".$syear."',fullname='".$fullname."',address='".$address."' WHERE mid='{$cfg_ml->M_ID}'";
			$querys = "UPDATE `{$membermodel->table}` set sex='".$sex."',syear='".$syear."',uname='".$uname."',fullname='".$uname."',mobile='".$phone."',address='".$address."' WHERE mid='{$cfg_ml->M_ID}'";
