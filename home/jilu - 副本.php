...
if($rank == 'top'){
	...
	//-------------------------------------------------------------------------------------冯type=1行业名企报名
    //$sql = "SELECT s.*,t.* FROM `#@__member_stow` AS s left join `#@__member_stowtype` AS t on t.stowname=s.type  where s.mid='".$cfg_ml->M_ID."'and s.type='1' order by s.id desc";
	$sql = "SELECT s.*,t.*,p.uname,p.sex,p.mobile,p.address FROM (`#@__member_stow` AS s left join `#@__member_stowtype` AS t on t.stowname=s.type) left join `#@__member_person` as p  on s.mid=p.mid  where  s.type='1' order by s.id desc";
    $tpl = 'jilu';
}
