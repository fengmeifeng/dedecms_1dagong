...
if(empty($dopost))
{
   ...
   $equery = "SELECT m.mid,m.jointime,m.uname,m.sphone,m.sphone2,c.*,m.spacesta FROM `#@__member` as  m
	...
	//echo  "<pre>";print_r($row);echo $row['introduce']; exit;
...
else if($dopost=='save')
{
   ...
   //include(dirname(__FILE__).'/inc/archives_check.php');
	...
	if(!empty($_POST)){
		...
		//--------------------spacesta='2'-------------------------------------------------------------
		$query = "UPDATE `#@__member` set qym='".$company."',uname='".$linkman."',sphone='".$sphone."',sphone2='".$mobile."',sex='".$sex."', shengfenid='".$shengfenid."',verify='1',xiaoshou='".$xiaoshou."',nativeplace='".$nativeplace."' WHERE mid='$mid'";
		//echo $query;exit;
