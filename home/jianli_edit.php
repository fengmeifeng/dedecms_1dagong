...
if(empty($dopost))
{
    ...
    include(DEDEMEMBER."/mystyles/jianli_edit.htm");
    exit();
}
...
else if($dopost=='save')
{
	...
	//----------------------------更新时间
	$pubdate = time();
	//echo date('Y-m-d H:i',$pubdate);exit;
    //处理图片文档的自定义属性
    if($litpic!='') $flag = 'p';
	//--------------------------同事更新手机号---------------------------------
	...
	//------------------------------------------------------------冯	
	 ...
	 //echo $gztime."<br>";echo $lasttime;exit;
    //更新数据库的SQL语句
	...
	//echo $aid;exit;
    //返回成功信息
	 ShowMsg('修改成功','jianli_edit.php?channelid=82&aid='.$aid,0,3000);
   /* $msg = "　　请选择你的后续操作：
