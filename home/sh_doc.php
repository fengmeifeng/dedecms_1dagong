...
else if($dopost=="delArc")
{
    ...
    //删除会员信息
	$sql="delete  FROM `#@__member`  WHERE mid='$mid'";
	$rs = $db->ExecuteNoneQuery($sql); 
	$sql_company="delete  FROM `#@__member_company`  WHERE mid='$mid'";
	$rs_company = $db->ExecuteNoneQuery($sql_company);
	//--------fffff---删除企业会员对应发布的职位------冯-------------
	$sql_arc="delete  FROM `#@__archives`  WHERE mid='$mid'";
	$rs_arc = $db->ExecuteNoneQuery($sql_arc);
	//---------------------------------------------------------------
	//echo "232323";exit;
    if($rs && $rs_company && $rs_arc)
    {
        //更新用户记录
