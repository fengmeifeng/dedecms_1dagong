...
if(empty($dopost))
{
    ...
    $arcQuery = "SELECT arc.*,ch.addtable,ch.fieldset,arc.mtype as mtypeid,ch.arcsta
       ...
       WHERE arc.id='$aid' And arc.mid='".$mid."'; ";
	...
	//------------------------------------------------------------------冯-----检索出公司名称
	$row_company = $dsql->GetOne("SELECT `company`,`comface` FROM #@__member_company WHERE mid='$mid'");
...
else if($dopost=='save')
{
    ...
    //返回成功信息
	 ShowMsg('修改成功','shenhe.php?channelid=81',0,3000);
}
//----------------------------------------------------------------------------冯
	 /*------------------------------
function _SaveArticle(){  }
------------------------------*/
	...
	//-------------------------------------------------------------------------------------------------冯
	$row_arc = $dsql->GetOne("Select `typeid`,`litpic`,`title` From `#@__archives` where id='$aid' and mid='$mid'");
	$title = $row_arc['title'];
    ...
    //返回成功信息
	 ShowMsg('刷新职位成功','shenhe.php?channelid=81',0,3000);
   /* $msg = "　　请选择你的后续操作：
