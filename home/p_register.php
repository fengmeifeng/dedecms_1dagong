...
if($step == 2)
{
    ...
    if($dopost=='regbase')
    {
       ...
       /* if(!CheckEmail($email))
		...
		//echo $pwd;exit;
		//diqu 
		if(empty($uname)){ShowMsg('请填写您的称呼！', '-1');exit();}
		$diqushengtext=$_POST['diqushengtext'];
		$diqushitext=$_POST['diqushitext'];
		$address=$diqushengtext.$diqushitext;
		//------------------ffff--必选择--------------------
		if(empty($diqushengtext) || empty($diqushitext) || $diqushengtext=='0' || $diqushitext=='0'){
			//showmsg('请选择地区的省或者市级', 'javascript:history.back()');exit;
			echo "<script language='javascript' type='text/javascript'>alert('请选择目前居住地址的省份或者城市!');window.history.back(-1);</script>";exit;
		}
		//echo $address;exit;
       ...
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";
        ...
        if($dsql->ExecuteNoneQuery($inQuery))
        {
			...
			*/
			...
			$url="http://60.173.200.172:61500/sdk/infoGetter.ashx?fun=set_cstm_info&templet=2&batch=1&col=field_text_001,field_text_003,field_text_005,field_text_007,field_text_013&info='".$uname."','".$userid."','".$syear."','".$address."','".$sex."'&tel_num=".$userid."&corp_code=1009";			
