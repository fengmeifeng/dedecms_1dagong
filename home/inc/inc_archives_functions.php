...
//=================复制开始,用于"添加"自动显示hash==============
function PrintHuiyuan(&$fieldset, $loadtype='all', $isprint=TRUE)
{
    global $cfg_cookie_encode;
    $dtp = new DedeTagParse();
    $dtp->SetNameSpace('field','<','>');
...
//=================复制开始,用于文档修改========
function PrintHuiyuanXiugai(&$fieldset, &$fieldValues, $loadtype='all')
{
    $dtp = new DedeTagParse();
    $dtp->SetNameSpace("field","<",">");
    $dtp->LoadSource($fieldset);
...
/*function GetUpdateTest()
{
    ...
    if($cfg_makeindex=='Y' || $cfg_make_andcat=='Y' || $cfg_make_prenext=='Y')
    {
		...
		//echo "aid=".$arcID."<br>";echo "dopost=".$dolists[0]."<br>";exit;
