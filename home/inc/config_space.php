...
if($row_space['rank'] == '20000'){$_vars['spacesta'] = 2;}
//-------------------------------------------------------------
if($_vars['spacesta'] < 0)
{
...
function GetUserSpaceInfos()
{
    ...
    $query = "SELECT m.mid,m.mtype,m.userid,m.uname,m.sex,m.rank,m.email,m.scores,
                            ...
                            s.*,t.*,m.matt,r.membername,g.msg,zh.zhizhao
                  From `#@__member` m
				  ...
				  LEFT JOIN `#@__diyzhizhao` zh on zh.mid=m.mid
                  where m.mid like '$uid' ORDER BY g.dtime DESC ";
