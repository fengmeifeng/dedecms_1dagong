 ...
 */
 //------------------------------------------------------------------行业名企栏目二级栏目调用冯
function GetOptionList($selid=0, $channeltype=0,$reid=0)
{
    ...
    $OptionArrayList = "";
    $query = "SELECT id,typename,ispart,channeltype,issend FROM `#@__arctype` WHERE ispart<2 AND reid=$reid ORDER BY sortrank ASC ";
    $dsql->SetQuery($query);
    $dsql->Execute();
    $selected = '';
    while($row=$dsql->GetObject())
