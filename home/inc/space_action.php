...
//-----------------冯查看权限-审核员可以查看公司发布的未审核的职位-----------------------
$row_space = $dsql->GetOne("SELECT `rank` FROM #@__member WHERE mid='".$cfg_ml->M_ID."'");//----------------feng
if($row_space['rank'] == '20000' || $row_space['rank'] == '30000'){$where = " >= ";}else{$where=" > ";}
//-----------------------------------------------------------------------------2015/4/6--
if($cfg_mb_allowncarc=='N') $addqSql .= " And arc.arcrank ".$where." -1 ";
...
else if($action=='archives')
{
    ...
    else
    {
        ...
        {
			//echo "2";exit;
            $query = "SELECT arc.*,mt.mtypename,tp.typedir,zh.zhizhao,tp.typename,tp.isdefault,tp.defaultname,tp.namerule,tp.namerule2,tp.ispart,tp.moresite,tp.siteurl,tp.sitepath,g.gongzi
                    from `#@__archives` arc
                    LEFT JOIN `#@__arctype` tp on arc.typeid=tp.id
                    LEFT JOIN `#@__mtypes` mt on mt.mtypeid=arc.mtype LEFT JOIN `#@__addgongzuo81` as g on g.aid=arc.id LEFT JOIN `#@__diyzhizhao` zh on zh.mid=arc.mid
    ...
    $dlist->pageSize = $_vars['pagesize'];
    $dlist->SetParameter("mtype", $mtype);
    $dlist->SetParameter("uid", $_vars['mid']);//------------userid
    $dlist->SetParameter("channelid", $channelid);
    $dlist->SetParameter("action", $action);
	//---------------------------------------------------------------------------------------
    //$dlist->SetTemplate(DEDEMEMBER."/space/{$_vars['spacestyle']}/listarchives.htm");
	$dlist->SetTemplate(DEDEMEMBER."/space/{$_vars['spacestyle']}/company.htm");
	//---------------------------------------------------------------------------------------
    $dlist->SetSource($query);
