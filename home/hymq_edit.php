...
if(empty($dopost))
{
    ...
    include(DEDEMEMBER."/mystyles/hymq_edit.htm");
    exit();
}
...
else if($dopost=='save')
{
	...
	ShowMsg('修改成功','hymq.php?channelid=66',0,3000);
    //返回成功信息
   /* $msg = "　　请选择你的后续操作：
...
else if($dopost=='pubArc')
{
	...
	//---------------------------------------------峰
	 $pubdate = time();
    //更新数据库的SQL语句
    $upQuery = "UPDATE `#@__archives` SET pubdate = '$pubdate' WHERE id='$aid' And mid='$cfg_ml->M_ID'; ";
    ...
    //返回成功信息
	 ShowMsg('刷新成功','hymq.php?channelid=66',0,3000);
}