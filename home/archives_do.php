...
else if($dopost=="viewArchives")
{
    ...
    CheckRank(0,0);
    if($type==""){
        header("location:".$cfg_phpurl."/viewvip.php?aid=".$aid."&cid=".$cid);
    }else{
...
删除上传的附件
----------------*/
else if($dopost=="delzhizhao")
{
	
    
    if($id !='')
    {
        $arow = $dsql->GetOne("SELECT * FROM `#@__diyzhizhao` WHERE id='$id'; ");
        if(is_array($arow) && $arow['mid']==$cfg_ml->M_ID)
        {
			//print_r($arow);exit;
            $res=$dsql->ExecuteNoneQuery("DELETE FROM `#@__diyzhizhao` WHERE id='$id'; ");
            if($res)
            {
               ShowMsg("成功删除上传的图片！",'/home/zhizhao.php');
				exit();
            }else{
				ShowMsg("删除失败！",'/home/zhizhao.php');
				exit();
			}
        }
	}
