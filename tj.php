<?php
	require_once(dirname(__FILE__)."/home/config.php");
...
if($dopost=='')
{
	...
	if(!empty($_GET['bh'])){
		...
		$user=$db->getone("select * from tuijian where bianhao='".$bh."' ");			//查询数据库
		if(!empty($user)){
			...
			require_once(dirname(__FILE__)."/home/12tuijian/style/tj.htm");
			exit();
		}else{
			showmsg('链接有误，正在为你跳转到壹打工首页! ', 'index.php');
			exit();
		}
	}else{
		showmsg('链接有误，正在为你跳转到壹打工首页! ', 'index.php');
...
//保存数据	
		...
		if(empty($userdata)){
			//接收post过来的编号
			$bianhao=base64_decode($bianhao);
			//查询这个推荐人是否存在
			$user=$db->getone("select * from tuijian where bianhao='".$bianhao."' ");
			...
			if(!empty($user)){
				...
				if($id > 0){
					...
					//--发短信提醒被推荐人的1打工网的帐号-------------------------------------------------
					$content="您好".$name."，您已成功通过".$user['name']."的推荐加入1+2事业平台，请登陆www.1dagong.com查看。账号为您的手机号，密码为手机号后四位。【壹打工网】";
				...
				showmsg('没有找到推荐你的人，请检测链接是否有误! ', '-1');
				exit();
			}
			
		}else{
			showmsg('您已经注册过了，只要成功入职，就可以推荐别人拿到推荐奖金', '-1');
			exit();
		}
}