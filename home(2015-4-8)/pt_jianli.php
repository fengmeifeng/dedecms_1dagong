...
if(empty($dopost))
{
	...
	if($row1['mtype'] == "个人"){
		//------------------------------------------------------------------冯-----检索出个人会员姓名
	$row_person = $dsql->GetOne("SELECT `fullname`,`sex`,`nativeplace`,`address` FROM #@__member_person WHERE mid='$cfg_ml->M_ID'");
	//echo $row_person['sex'];exit;
    include(DEDEMEMBER."/mystyles/pt_jianli.htm");//===========================
	}
	else{
	ShowMsg("对不起，您不是企业用户不能发布简历！","-1","0",5000);
	}
    exit();
}
/*------------------------------
...
else if($dopost=='save')
{
    ...
    if(!empty($dede_addonfields))
    {
        ...
        //echo $dede_fieldshash."<br>";echo md5($dede_addonfields.$cfg_cookie_encode);exit;
	...
	//------------------------------------------------------------冯	
	 $gztime = strtotime($_POST['gztime']);
	 $lasttime = strtotime($_POST['lasttime']);

	 //echo $arcrank;exit;//----------------------------------简历不需要审核
	 $arcrank = 0;
	 $writer=$title;//echo $writer;exit;
	 //--------------------------同事更新手机号---------------------------------
	$dsql->ExecuteNoneQuery("UPDATE `#@__member` SET sphone='$phone',sex='$sex' WHERE  mid ='$cfg_ml->M_ID';");
	$dsql->ExecuteNoneQuery("UPDATE `#@__member_person` SET mobile='$phone',sex='$sex' WHERE  mid='$cfg_ml->M_ID';");
	 //-------------------------------------------------------------------------
...
'$color','$writer','$source','$litpic','$pubdate','$senddate','$mid','$description','$keywords','$mtypesid'); ";
	...
	//-----------------------------------------------搜索aid
	$a = $dsql->GetOne("Select a.id From `#@__archives` as a left join `#@__addjianli82` as b on a.id=b.aid WHERE  a.mid='$cfg_ml->M_ID'");
	//echo "<pre>";print_r($a);exit;
     //返回成功信息
   ShowMsg('简历发布成功','jianli_edit.php?channelid=82&aid='.$a['id'],0,3000);
    /*$msg = "
    　　请选择你的后续操作：
