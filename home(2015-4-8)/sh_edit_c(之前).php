...
{
    //读取归档信息
   $equery = "SELECT m.mid,m.jointime,m.uname,c.company,m.spacesta,c.* FROM `#@__member` as  m
...
else if($dopost=='save')
{
   ...
   //include(dirname(__FILE__).'/inc/archives_check.php');
	...
	if(!empty($_POST)){
		...
		//--------------------spacesta='2'-------------------------------------------------------------
		$query = "UPDATE `#@__member` set uname='".$linkman."',sphone='".$phone."',sex='".$sex."', shengfenid='".$shengfenid."',verify='1',xiaoshou='".$xiaoshou."',nativeplace='".$nativeplace."' WHERE mid='$mid'";
		//echo $query;exit;
		if($dsql->ExecuteNoneQuery($query)){
			...
			//处理图片文档的自定义属性
			if($litpic!='') $flag = 'p';
			//---------------------更新archives主表和字表-writer和gzname------------------
			$dsql->ExecuteNoneQuery("UPDATE `#@__archives` set writer='".$company."' WHERE mid='$mid'");
			//------------------------------------------------------------------------------
			
			$querys = "UPDATE `#@__member_company` set sex='".$sex."',linkman='".$linkman."',email='".$email."',zhiwei='".$zhiwei."',company='".$company."',mobile='".$phone."',nativeplace='".$nativeplace."',comface='".$comface."',xiaoshou='".$xiaoshou."' WHERE mid='$mid'";
			//echo $querys;exit;
