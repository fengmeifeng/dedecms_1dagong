...
else if($action=='archives')
{
    ...
    else
    {
        ...
        {
			//echo "2";exit;
            $query = "SELECT arc.*,mt.mtypename,tp.typedir,tp.typename,tp.isdefault,tp.defaultname,tp.namerule,tp.namerule2,tp.ispart,tp.moresite,tp.siteurl,tp.sitepath,g.gongzi
                    from `#@__archives` arc
                    LEFT JOIN `#@__arctype` tp on arc.typeid=tp.id
                    LEFT JOIN `#@__mtypes` mt on mt.mtypeid=arc.mtype LEFT JOIN `#@__addgongzuo81` as g on g.aid=arc.id
