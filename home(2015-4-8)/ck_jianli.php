...
if($rank == 'top'){
    ...
    $sql = "SELECT s.*,t.* FROM `#@__member_stow` AS s left join `#@__member_stowtype` AS t on t.stowname=s.type  where s.company_id='".$cfg_ml->M_ID."' order by s.id desc";
    $tpl = 'ck_jianli';
}
...
$dlist->pageSize = 20;
$dlist->SetTemplate(DEDEMEMBER."/templets/ck_jianli.htm");
$dlist->SetSource($sql);
$dlist->Display();