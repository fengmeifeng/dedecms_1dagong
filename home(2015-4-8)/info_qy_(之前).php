...
if($dopost=='save'){
	...
	*/	
		...
		//-----------spacesta='2'---------------------------------------------------------------------------
		$query = "UPDATE `#@__member` set uname='".$linkman."',sphone='".$phone."',sex='".$sex."', shengfenid='".$shengfenid."',verify='1',xiaoshou='".$xiaoshou."',nativeplace='".$nativeplace."' WHERE mid='{$cfg_ml->M_ID}'";
		//echo $query;exit;
		if($dsql->ExecuteNoneQuery($query)){
			...
			//echo $litpic;exit;
			$querys = "UPDATE `{$membermodel->table}` set sex='".$sex."',linkman='".$linkman."',email='".$email."',zhiwei='".$zhiwei."',company='".$company."',mobile='".$phone."',nativeplace='".$nativeplace."',comface='".$comface."',xiaoshou='".$xiaoshou."' WHERE mid='{$cfg_ml->M_ID}'";
