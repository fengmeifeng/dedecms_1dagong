...
//判断个人和企业-----------------------------------------------------------------------------------------------------冯
	...
	if($row1['mtype'] == "企业"){
		...
		ShowMsg('对不起,您不是个人用户，不允许报名','javascript:window.history.back()');exit();
...
if($type==''){
	...
	if(!is_array($row)){
  	$dsql->ExecuteNoneQuery(" INSERT INTO `#@__member_stow`(mid,aid,title,addtime,type,company_id,company,person,p_aid) VALUES ('".$ml->M_ID."','$aid','$title','$addtime','$type','$job','$title','".$person['uname']."','".$row2['id']."'); ");
  }
}
...
ShowMsg('对不起,您已报名','javascript:window.history.back()');
}
