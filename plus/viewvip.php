...
$t1 = ExecTime();*/
//-------------------------------------------------------------------------审核员查看权限
$row_view = $dsql->GetOne("SELECT `rank` FROM #@__member WHERE mid='".$cfg_ml->M_ID."'");//----------------feng
if($row_view['rank'] == '20000' || $row_view['rank'] == '30000'){$needRank = 0;}
//-------------------------------------------------------------------------审核员查看权限
if($needRank < 0 && $arc->Fields['mid'] != $cfg_ml->M_ID)
{
   // ShowMsg('文章尚未审核,非作者本人无权查看!', 'javascript:;');
   ShowMsg('内容未经审核，非发布者无查看权限！', 'javascript:;');
    exit();
}
...
if($cfg_ml->IsLogin()){
    // echo $cfg_ml->M_ID;exit;
    if($row_view['rank'] == '20000' || $row_view['rank'] == '30000'){
        $arc->Display();
    }else{
        $query="select m.* from `#@__member_stow` as m where m.p_aid=$aid and m.company_id=$cfg_ml->M_ID";
        $dsql->SetQuery($query);
        ...
        showmsg('您还没有登录，请先登录','/home/index.php');
}