...
//将搜索类别【公司名|职位名】保存 By Z
$GLOBALS['searchtype']=$searchtype;
// var_dump($GLOBALS['searchtype']);exit;
if(!isset($keyword)){
    if(!isset($q)) $q = '';
    $keyword=$q;
}
...
if(($keyword=='1' || strlen($keyword)<0) && empty($typeid))
{
...
//开始时间
if(empty($starttime)) $exstarttime = -1;
else
{
    ...
    if($starttime>0)
    {
       ...
       $exstarttime = time() - ($starttime * $dayst);
  }
}
//薪酬范围By Z
...
// echo $GLOBALS['areacode'];exit;
$t1 = ExecTime();

$sp = new SearchView($typeid,$keyword,$orderby,$channeltype,$searchtype,$exstarttime,$pagesize,$kwtype,$mid,$salary,$areacode);
$keyword = $oldkeyword;
$GLOBALS['num_count']=$sp->TotalResult;
$GLOBALS['typeid']=$typeid;
$GLOBALS['keyword']=$keyword;
// echo $GLOBALS['starttime'];exit;
$sp->Display();
