...
if(empty($dopost))
{
    ...
    include(DEDEMEMBER."/mystyles/pt_job.htm");
    exit();
}
/*------------------------------
...
'$color','$writer','$source','$litpic','$pubdate','$senddate','$mid','$description','$keywords','$mtypesid'); ";
    ...
    if(empty($addtable))
    {
        ...
        //$inquery = "INSERT INTO `{$addtable}`(aid,typeid,userip,redirecturl,templet{$inadd_f}) Values('$arcID','$typeid','$userip','',''{$inadd_v})";
       	$inquery = "INSERT INTO `{$addtable}`(aid,typeid,userip,redirecturl,templet{$inadd_f},jz_leixing,yuan,jz_daiyu,jz_jiesuan,jz_sex,jz_shengao,jz_jiankangzheng,jz_renwu,jz_firsttime,jz_lasttime,jz_didian,jz_gzshijian,jz_bmrenshu,jz_gzfangshi) Values('$arcID','$typeid','$userip','',''{$inadd_v},'$jz_leixing','$yuan','$jz_daiyu','$jz_jiesuan','$jz_sex','$jz_shengao','$jz_jiankangzheng','$jz_renwu','$jz_firsttime','$jz_lasttime','$jz_didian','$jz_gzshijian','$jz_bmrenshu','$jz_gzfangshi')";
	    if(!$dsql->ExecuteNoneQuery($inquery))
        {
            $gerr = $dsql->GetError();
            $dsql->ExecuteNoneQuery("Delete From `#@__archives` where id='$arcID'");
   ...
   /* $msg = "
    　　请选择你的后续操作：
        <a href='pt_job.php?cid=$typeid&channelid=$channelid'><u>继续发布内容</u></a>
        &nbsp;&nbsp;
        <a href='$artUrl' target='_blank'><u>查看内容</u></a>
        &nbsp;&nbsp;
        <a href='jianzhi_edit.php?channelid=$channelid&aid=$arcID'><u>更改内容</u></a>
        &nbsp;&nbsp;
        <a href='jianzhi.php?channelid={$channelid}'><u>已发布内容管理</u></a>
        ";
	...
	ShowMsg("发布工作信息成功","jianzhi.php?channelid={$channelid}","0",1000);
}