...
if($step == 2)
{
    ...
    if($dopost=='regbase')
    {
       ...
       /* $svali = GetCkVdValue();
        ...
        if(strlen($userid) > 15 || strlen($uname) > 40)
        {
        ...
        $inQuery = "INSERT INTO `#@__member` (`mtype` ,`userid` ,`pwd` ,`uname` ,`sex` ,`sphone`,`qym`,`form`,`rank` ,`money` ,`email` ,`scores` ,
        `matt`, `spacesta` ,`face`,`safequestion`,`safeanswer` ,`jointime` ,`joinip` ,`logintime` ,`loginip` )
       VALUES ('$mtype','$userid','$pwd','$uname','$sex','$sphone','$qym','$address','10','$dfmoney','$email','$dfscores',
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";
        ...
        if($dsql->ExecuteNoneQuery($inQuery))
        {
            ...
            if($mtype=='个人'){
			 ...
			 //写入默认企业注册表
			 if($mtype=='企业'){
            $membertjquery = "INSERT INTO `#@__member_company` (`mid`,`company`,`linkman`,`mobile`,`address`,`email`,`zhiwei`,`sex`)
                   VALUES ('$mid','$qym','$uname','$sphone','$address','$email','$zhiwei','$sex')";
			...
			*/
            ...
            if($cfg_mb_reginfo == 'Y' && $spaceSta >=0)//判断会员时候完善信息
            {
				...
				ShowMsg("您以成功注册！，请登录.....","index_company.php",0,1000);
                exit();
            } else {
				if($cfg_ml->IsLogin())
				{
