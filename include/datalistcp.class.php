	...
	function RemoveXss($val) {
		...
		while ($found == true) {
		  ...
		  for ($i = 0; $i < sizeof($ra); $i++) {
			 ...
			 $pattern .= '/i';
			 $replacement = substr($ra[$i], 0, 6).'<x>'.substr($ra[$i], 6);
        ...
        if(count($this->getValues)>0)
        {
            foreach($this->getValues as $key=>$value)
            {
				...
				if(isset($_GET['company_name']) && $_GET['company_name']!=''){
					$c="&company_name=".$_GET['company_name'];
				}
				if(isset($_GET['start']) && isset($_GET['end']) && $_GET['start']!='' && $_GET['end']!=''){
					$start_end="&start=".$_GET['start']."&end=".$_GET['end'];
				}
				if(isset($_GET['shenhe_sta']) && $_GET['shenhe_sta']!=0){
					$shenhe="&shenhe_sta=".$_GET['shenhe_sta'];
				}
				if(isset($_GET['zhizhao_sta']) && $_GET['zhizhao_sta']!=-1){
					$z="&zhizhao_sta=".$_GET['zhizhao_sta'];
				}
				//增加一个销售姓名搜索
				if(isset($_GET['xiaoshou']) && $_GET['xiaoshou']!=''){
					$x="&xiaoshou=".$_GET['xiaoshou'];
				}
                $geturl .= "$key=$value".$c.$x.$start_end.$shenhe.$z."&";
				/*ffffff分页ffffffff*/
