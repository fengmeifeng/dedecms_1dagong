...
function CheckUserID($uid, $msgtitle='用户名', $ckhas=TRUE)
{
    ...
    if($cfg_md_idurl=='Y' && preg_match("/[^a-z0-9]/i",$uid))//-原来备份--/[^a-z0-9]/i
    {
        return $msgtitle.'必须由英文字母或数字或者下划线组成！';
    }
    ...
    for($i=0; isset($ck_uid[$i]); $i++)
    {
        ...
        {
            if(preg_match("/[^0-9a-z@\.-_]/i",$ck_uid[$i]))//--原备份-/[^0-9a-z@\.-]/i------ffff添加一个下划线可以通过fff-
            {
    ...
    if($ckhas)
    {
        ...
        if(is_array($row)) return $msgtitle."1";			//用户名已经存在---------------徐阳修改
    }
    return 'ok';
}
...
function CheckNotAllow()
{
    ...
    {
        ShowMsg("未完善会员资料，还不可以加入。", "/home/info.php");
        exit();
    }
...
class MemberLogin
{
    ...
    function __construct($kptime = -1, $cache=FALSE)
    {
        ...
        if(empty($this->M_ID))
        {
            ...
            if(is_array($this->fields)){
				...
				//----添加-----------------------------
				$this->M_yixianggongsi = $this->fields['yixianggongsi'];         //意向公司-----徐阳添加
				//------------------------end----------
    ...
    function ResetUser()
    {
        ...
        $this->M_HasDay = 0;
		//-----添加-----------------------------
		$this->M_yixianggongsi = '';				//意向公司----徐阳添加
		//---------------------end--------------
    ...
    function CheckUser(&$loginuser, $loginpwd)
    {
        ...
        //$row = $dsql->GetOne("SELECT mid,matt,pwd,logintime FROM `#@__member` WHERE userid LIKE '$loginuser' ");		--原代码
		//修改可以使用手机号登录
		$row = $dsql->GetOne("SELECT mid,matt,pwd,logintime FROM `#@__member` WHERE userid LIKE '$loginuser' or sphone='$loginuser' ");		//--修改人徐阳
