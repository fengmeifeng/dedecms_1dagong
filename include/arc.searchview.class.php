...
class SearchView
{
    ...
    $searchtype='',$exstarttime=0,$upagesize=20,$kwtype=1,$mid=0,$salary,$areacode)
    {
    ...
    $searchtype="",$exstarttime=0,$upagesize=20,$kwtype=1,$mid=0)
    {
        $this->__construct($typeid,$keyword,$orderby,$achanneltype,$searchtype,$exstarttime,$upagesize,$kwtype,$mid);
    }
    ...
    function GetKeywordSql()
    {
        ...
        foreach($ks as $k)
        {
            ...
            if($this->ChannelType < 0 || $this->ChannelTypeid < 0){
                ...
                elseif($this->SearchType=="comtype"){
                    $kwsqls[] = " a.gzname LIKE '%$k%' ";
                }else{
                    // $kwsqls[] = " CONCAT(arc.title,' ',arc.writer,' ',arc.keywords) LIKE '%$k%' ";
                    $kwsqls[] = " CONCAT(arc.title,' ',arc.keywords) LIKE '%$k%' ";
                }
            }
        }
        if(!isset($kwsqls[0]))
    ...
    function CountRecord()
    {
        ...
        $ksqls = array();
        if(!empty($this->Salary)&&isset($this->Salary)){
            $ksqls[] = "a.gongzi='".$this->Salary."' ";
        }
        /*if($this->StartTime > 0)
        {
        ...
        if($this->StartTime > 0)
        {
            $ksqls[] = " arc.pubdate>'".$this->StartTime."' ";
        }
        //#########################################################################根据地区条件来筛选职位信息 By Z
        if($this->Areacode > 0)
        {
            switch ($this->Areacode){
                case 1:
                    $ksqls[] = " a.nativeplace=10501 ";
                    break;
                case 2:
                    $ksqls[] = " a.nativeplace in(1001,1002,1003,1004,1005,1006,1007,1008,1009,1010,1011,1012,1013,1014,1015,1016,1017,1018,1019) ";
                    break;    
                case 3:
                    $ksqls[] = " a.nativeplace in(2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025,2026,2027,2028,2029,2030,2031,2032,2033,2034,2035,2036,2037,2038,2039,2040) ";
                    break;
                case 4:
                    $ksqls[] = " a.nativeplace in(4001,4002,4003,4004,4005,4006,4007,4008,4009,4010,4011,4012,4013) ";
                    break;
                case 5:
                    $ksqls[] = " a.nativeplace in(7001,7002,7003,7004,7005,7006,7007,7008,7009,7010,7011,7012,7013,7014,7015,7016,7017) ";
                    break;
            }
        }
        //已过期，即当前时间大于截至时间的职位不予显示
        $this->Systime=time();
        $ksqls[] = " a.lasttime >'".$this->Systime."' ";
        if($this->TypeID > 0)
        {
        ...
        $ksqls[] = " arc.arcrank > -1 ";
		//########################## 增加搜索条件：筛选职位，即channel_id为81的结果 By Z
		$ksqls[] = " arc.channel = 81 ";
        ...
        $cquery = "SELECT * FROM `{$this->AddTable}` arc LEFT JOIN `#@__addgongzuo81` a ON arc.id=a.aid
            WHERE {$this->AddSql}";
        //打印查询语句，这里的查询是为了给出查询的结果计数 By Z
        // echo '===';var_dump($cquery);exit;
        // echo '1';exit;
        //var_dump($cquery);
        $hascode = md5($cquery);
    ...
    $imgwidth=120,$imgheight=90,$achanneltype="all",$orderby="default",$innertext="",$tablewidth="100")
    {
        ...
        if ($cfg_sphinx_article == 'Y')
        {
            ...
            // echo 'aaa';exit;
            //sql搜索语句 By Z 按更新时间arc.pubdate排序
            $query = "SELECT arc.*,act.typedir,act.typename,act.isdefault,act.defaultname,act.namerule,
            act.namerule2,act.ispart,act.moresite,act.siteurl,act.sitepath
            FROM `{$this->AddTable}` arc LEFT JOIN `#@__arctype` act ON arc.typeid=act.id LEFT JOIN `#@__addgongzuo81` a ON arc.id=a.aid
            WHERE {$this->AddSql} ORDER BY arc.pubdate desc LIMIT $limitstart,$row";    
            // echo $query;exit;
        }
        ...
        for($i=0;$i<$row;$i++)
        {
            ...
            for($j=0;$j<$col;$j++)
            {
                ...
                if($row = $this->dsql->GetArray("al"))
                {
                    ...
                    //=================2015-03-12 17-12-39========搜索页面调用自定义字段====start====这是第二处，第一处在include/extend.func.php.php==========
                    $row["zwmiaoshu"]=Search_addfields($row["id"],"zwmiaoshu");
                    //=================2015-03-12 17-12-39========搜索页面调用自定义字段====end====这是第二处，第一处在include/extend.func.php.php==========
                    $row["gzname"]=Search_addfields($row["id"],"gzname");
                    $row["gongzi"]=Search_addfields($row["id"],"gongzi");
                    $row["bmrenshu"]=Search_addfields($row["id"],"bmrenshu");
					$row["zprenshu"]=Search_addfields($row["id"],"zprenshu");
                    $row["nativeplace"]=Search_addfields($row["id"],"nativeplace");
	 ...
	 function GetPageListDM($list_len)
    {
        ...
        {
            return "<li><a href='#'>共1页/".$this->TotalResult."条记录</a></li>\r\n";
        }
        if($this->TotalResult == 0)
        {
            //return "<li><a href='#'>共0页/".$this->TotalResult."条记录</a></li>\r\n";
			return "<li><a href='#'>对不起，您没有找到相关信息</a></li>\r\n";
        }
        $purl = $this->GetCurUrl();
		...
		$geturl .="areacode=".$this->Areacode."&starttime=".$this->StartTime."&";
        $purl .= "?".$geturl;
        ...
        {
            $indexpage="<li class=\"disabled\"><a href='#'>«</a></li>";
        }
        ...
        {
            $endpage="<li class=\"disabled\"><a href='#'>»</a></li>";
        }
