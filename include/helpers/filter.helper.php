...
//手机号码的验证过滤
if(! function_exists('MobileNumValidate')){
  function MobileNumValidate($str){
    if(preg_match('/[^\d]/',$str)){
      ShowMsg('请输入11位正确的手机号码！',-1);
      exit();
    }
  }
}
...
//汉字的过滤（不允许出现<>?$;{}）
if(! function_exists('TextValidate')){
  function TextValidate($str){
    if(preg_match('/[\<\>\?\$\;\{\}]/',$str)){
      ShowMsg('您的输入中包含非法字符，请按照提示正确输入！',-1);
      exit();
    }
  }
}
...
//字母数字下划线之外的字符过滤
if(! function_exists('OtherValidate')){
  function OtherValidate($str){
    if(preg_match('/[^\w]/',$str)){
      ShowMsg('您的输入中包含非法字符，请按照提示正确输入！',-1);
      exit();
    }
